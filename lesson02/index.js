...
let a = 'some string',
    ...
    c = true,
    d = null;
console.log(Boolean(a), String(a), Number(a));
console.log(Boolean(b), String(b), Number(b));
console.log(Boolean(c), String(c), Number(c));
console.log(Boolean(d), String(d), Number(d));
...
const b1 = 3;
const b2 = 4;
const k = Math.sqrt(Math.pow(b1, 2) + Math.pow(b2, 2));
console.log(k);
...
/** 
 * Задание 6
 * Запросите у пользователя 2 числа с помощью prompt и положите эти значения в переменные. 
...
const aa = prompt("введите a");
const bb = prompt("введите b");
if (Number.isNaN(Number(aa)) || Number.isNaN(Number(bb))) {
  alert ('Произошла ошибка');
} else if (aa === bb) {
  alert('чиала равны: ' + aa);
} else {
  alert(Math.max(aa, bb));
}
...
const fff = confirm('Хотите получить доступ?');

if (fff) {
  const age = prompt('возраст');
  alert(Number(age) >= 18 ? 'Доступ разрешен' : 'Доступ запрещен');
}
...
if (Number.isNaN(Number(vv)) || Number.isNaN(Number(cc))) {
  alert('Произошла ошибка');
} else {
  alert(vv % cc);
}
